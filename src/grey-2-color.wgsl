...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    var grey_color: vec3<f32> = vec3<f32>(value, value, value);
    var delta = s.rgb - grey_color;

    var resolution_value = resolution.value;
    if (resolution.reversed != 0.0) {
        resolution_value = 1.0 - resolution_value;
    }

    return vec4<f32>(grey_color + (resolution_value * delta), 1.0);
}
